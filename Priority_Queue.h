...
Process dequeue_PQ(PriorityQueue *pq) {
    if (pq->size == 0) {
        Process null_process = {0};  // Return a default Process if the queue is empty
        return null_process;
    }

    // The process with the highest priority is at the front of the queue
    Process highest_priority_process = pq->processes[0];
